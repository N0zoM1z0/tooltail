...
Folder authority can come from the deterministic Tooltail safe lab or an existing local fixed-drive folder. Existing-folder selection is non-authoritative: Home first shows the exact canonical root, closed action set, and five-minute preview, then a separate confirmation revalidates root identity and creates one seven-day grant. The restart root is stored only as current-user protected local state; failure to restore disables file work while preserving exact revocation. See [ADR 0011](docs/adrs/0011-explicit-existing-folder-grant-and-protected-root.md).

Support diagnostics are also local and explicit: Home builds a closed summary containing only typed states, stable reason codes, and bounded counts, then shows the exact JSON and SHA-256 before a one-time `CreateNew` export. It contains no raw path, filename, title, content, model text, credential, companion/user/machine identity, and has no uploader. ADR 0010 explains why whole-memory deletion remains the only safe v0.1 erasure boundary.
