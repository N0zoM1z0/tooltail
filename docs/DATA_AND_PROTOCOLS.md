...
The explicit production action reloads that planned document and requires byte-for-byte canonical JSON plus fingerprint equality. It persists the same immutable SkillVersion's legal `draft` to `approved` transition and then one `production`-purpose approval. Journal creation consumes the persisted approval in the same transaction that opens the execution. The executor's authority source reloads the exact skill version and grant from bounded SQLite projections before preparation, immediately after preparation and before the native effect, and again for verification, so revocation or lifecycle drift is immediately visible at the authorization line. A verified standard receipt retains exact destination identity/hash evidence, native mutation ownership evidence, and a bounded Undo window; after receipt storage the version advances to `practiced`.

Desktop Undo first reloads the standard receipt and journal and stores a reverse-ordered canonical `recovery` plan derived against a current authoritative snapshot. Preview creates no approval and performs no mutation. A later explicit decision reloads that recovery document plus the original evidence, stores one `undo`-purpose approval, and opens a distinct recovery journal. Internal removal of a copied file or `ensure_directory` target is allowed only when receipt evidence proves that the destination was created by that exact execution and the current identity/hash/emptiness proof still matches. On verified completion, every original step has an append-only rollback link to the recovery execution and a separate recovery receipt binds both plan fingerprints, both execution identities, exact verified recovery evidence, and an empty residual list. The original standard receipt is retained unchanged.

Windows native file identities are versioned. New native root, snapshot, plan, mutation, and receipt evidence uses `win32-volume-v2:` plus `win32-file-v2:` values captured from `FileIdInfo`. Older pre-alpha `win32-*` 64-bit identities are not compatibility-equivalent; readers fail closed during grant restore, plan authorization, or receipt/recovery proof and require explicit re-grant/re-plan rather than automatic migration.
...
Native v1 import is accepted by ADR 0009 only for the exact unique pristine first-run companion. A bounded local non-reparse file is captured once, parsed strictly, and shown with its exact SHA-256 before a separate commit. Import requires linear version histories and atomically replaces the empty identity plus all histories; every version is forced Stale and no authority/evidence trust is created. Existing state is never merged. If a future capsule becomes a multi-file archive, add a new schema/ADR and defend against traversal, duplicate entries, decompression bombs, links, and hash confusion.

The shared capsule service performs semantic validation before producing export bytes. It rejects unsafe content-policy flags, empty or duplicate identities, invalid or unsupported SkillSpecs, source-grant bindings that disagree with the SkillSpec, invalid evidence counters/timestamps, missing parents, and nonlinear/non-contiguous histories. It canonicalizes and orders immutable SkillSpecs before a bounded parser readback. A valid preview reports `createsAuthority = false`, `canImport = true`, and `skillsRequireRebind = true`; only the separate pristine-state transaction may persist it.

Desktop export loads the complete current skill histories, projects only bounded execution counts/timestamps, validates and readbacks the entire capsule, and then writes once with `CreateNew` below the Tooltail application-data `Exports` root. It never exports the physical lab root, grant capabilities/state, approvals, canonical plans, journals, receipts, Undo material, credentials, or file content. The locally displayed preview repeats `createsAuthority = false`, `canImport = true`, and `skillsRequireRebind = true`.

After import, explicit rebind reads one current Stale history and creates version `n + 1` with the newly issued grant. The only semantic-diff field is `scope_binding`; the result is Draft and must traverse the existing rehearsal, plan, approval, execution, and verification path. The imported parent and all historical IDs remain immutable.
...
Before the M6 research build was handed off or externally released, v1 added the closed `clarification_completed` and `approval_decided` discriminators required by the accepted study protocol. Existing events remain readable because their meanings and fields did not change; an older pre-handoff strict reader will reject these two later enum values and must not be used for M6 exports. Any discriminator change after M6 handoff requires a new schema version and explicit migration/reader policy.

The default contract contains random study/session/event identities, UTC timing, closed discriminators, stable reason codes, bounded numeric summaries, and optional session-salted equality tokens. Per-session sequence must start at zero and remain contiguous, and every line in one store must retain the consented study ID; strict readback rejects drift before another append or export. It has no raw path, filename, title, content, prompt, transcript, user, machine, credential, or free-form text field. JSONL export is locally previewed, created only on explicit request, and never uploaded automatically. The implementation data and its deletion policy are described by ADR 0007.
...
ADR 0011 uses the existing bounded `protected_canonical_root` grant blob for user-selected local roots. Picker selection is a non-authority preview; confirmation revalidates the same fixed-volume/non-reparse root identity and persists current-user DPAPI ciphertext, never a new external contract field. Restart decrypts and re-probes before use. Failure creates no implicit rebind: all file actions remain disabled while exact grant revocation stays available.

ADR 0010 keeps per-lesson, per-skill, and per-receipt deletion as future design targets because v1 immutable plans, append-only journals/receipts, SkillSpec provenance, and recovery links cannot be safely removed independently. Undo eligibility expiry prevents a new recovery authorization but does not erase durable evidence. Skill Card maintenance actions remain truthfully disabled; no documentation or UI claims automatic purge. Whole-memory deletion remains the exact erasure control.

The internal `tooltail.diagnostic-export/1` document contains UTC/product version, one typed body/tool snapshot, stable reason codes, and bounded aggregate counts for grants, current skill lifecycles, teaching evidence, execution state/receipts, recovery candidates, and local research status. It has explicit false content-policy flags and no field for paths, names, titles, content, model text, credentials, companion identity, user, or machine identity. Output is capped at 64 KiB, strictly read back, shown exactly with SHA-256, and written only with `CreateNew` below Tooltail-owned `Diagnostics`. It has no authority or uploader and is not a frozen external v1 schema.
