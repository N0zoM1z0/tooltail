...
| Companion Capsule continuity | PARTIAL | Authority-free export, strict bounded parsing, lineage, and no-authority tests pass. Native import/rebind UI is deliberately disabled, so the final export/import/rebind acceptance item is incomplete. |
| data export, deletion, and retention | PARTIAL | Capsule/research export and whole-product-memory deletion exist. Per-lesson, per-skill, per-receipt, automatic-retention, and diagnostic-export controls are not implemented. |
...
- There is no automatic retention scheduler and no individual deletion UI for lessons, skills, receipts, or executions. Until those exist, whole-memory deletion is the only product-state erasure control.
...
- Capsule files contain no grants or approvals and cannot restore authority. Although strict import parsing is tested, native import and rebind are not exposed in the product.
...
5. Implement and verify the remaining product acceptance gaps: native authority-free Capsule import/rebind and the required granular retention/deletion/export controls, or accept a documented scope change through an ADR without weakening safety invariants.
