...
| deterministic 2–5-example lesson, Skill Card, rehearsal, exact approval, execution, receipt, correction, reuse, and Undo | PASS in the bounded safe-lab engineering flow | The Desktop also ships exact preview/confirmation for one existing local fixed-drive folder, but that broader content/usability path has not received attended or participant evaluation. |
...
- The visible Desktop File Apprentice flow offers either a new Tooltail-owned synthetic safe lab or exact preview/confirmation for one existing local fixed-drive, non-reparse folder. Arbitrary folder contents may exceed snapshot/resource bounds or fail to form a supported deterministic lesson; selection grants no delete, overwrite, content-edit, network, shell, or approval authority.
